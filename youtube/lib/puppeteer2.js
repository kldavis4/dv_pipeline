...
(async () => {
  ...
  const videoTitle = filename.replace('-yt.mp4', '').replace(/_/g, ' ')
  const descFilename = filepath.replace('-yt.mp4', '-yt.txt')

  const clipPaths = fs.readFileSync(descFilename).toString().split('\n').filter(f => !!f)
  const lastPath = clipPaths[clipPaths.length - 1]

  const timestampRE = /clip-(?<year>[0-9]{4})-(?<month>[0-9]{2})-(?<day>[0-9]{2})_(?<hour>[0-9]{2})-(?<min>[0-9]{2})-(?<sec>[0-9]{2}).+/

  let description = clipPaths.map(clipPath => {
    const matches = timestampRE.exec(clipPath)
    return `${matches.groups['year']}-${matches.groups['month']}-${matches.groups['day']}T${matches.groups['hour']}:${matches.groups['min']}:${matches.groups['sec']}`
  }).join('\n')
  ...
  const matches = timestampRE.exec(lastPath)
  ...
  const descXPath = "//div[@aria-label='Tell viewers about your video (type @ to mention a channel)']"
  await page.waitForXPath(descXPath)

  let [descInput] = await page.$x(descXPath)
...
// contains(., 'Upload complete') or contains(., 'Processing complete') or 
  ...
  const shareUrlXPath = "//a[starts-with(@href,'https://youtu.be/')]"
